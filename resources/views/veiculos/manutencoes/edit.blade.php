...
@section('title', 'Editar manutenção - Veículos - Omega286')
@section('eyebrow', 'Operação / Veículos / Frota')
@section('page-title', 'Editar manutenção')

@section('content')
    <form method="POST" action="{{ route('veiculos.manutencoes.update', $manutencao) }}" enctype="multipart/form-data" class="space-y-6">
        @csrf
        @method('PUT')
