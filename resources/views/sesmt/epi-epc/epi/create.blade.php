...
@section('title', 'Novo EPI — SSMA - Omega286')
@section('eyebrow', 'SSMA / EPI-EPC')
@section('page-title', 'Novo registro de EPI')
...
@section('content')
    <section class="rounded-xl border border-zinc-200 bg-white p-6 shadow-sm md:p-8">
        <form method="POST" action="{{ route('sesmt.epi-epc.epi.store') }}" enctype="multipart/form-data" class="max-w-3xl">
            @csrf
            @include('sesmt.epi-epc.epi._form', ['epi' => $epi])
