...
@section('title', 'Editar EPI — SSMA - Omega286')
@section('eyebrow', 'SSMA / EPI-EPC')
@section('page-title', 'Editar EPI')
...
@section('content')
    <section class="rounded-xl border border-zinc-200 bg-white p-6 shadow-sm md:p-8">
        <form method="POST" action="{{ route('sesmt.epi-epc.epi.update', $epi) }}" enctype="multipart/form-data" class="max-w-3xl">
            @csrf
            @method('PUT')
