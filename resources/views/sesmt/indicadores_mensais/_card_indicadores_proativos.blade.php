@php
    ...
    $barChartAcoes = $card['barChartAcoes'] ?? [];
    ...
    $faixaRapida = $card['faixaRapida'] ?? [];
    $leituraProativos = $card['leituraProativos'] ?? '';
    $pontosProativos = $card['pontosProativos'] ?? [];
@endphp
    ...
    <div class="min-w-0 flex-1 basis-0">
        <div class="flex min-w-0 w-full gap-5">
            <div class="relative flex h-16 w-16 shrink-0 items-center justify-center rounded-full bg-[#600020] text-white shadow-md ring-4 ring-[#600020]/10">
                <i data-lucide="shield" class="h-9 w-9" stroke-width="1.5"></i>
                <span class="absolute -bottom-0.5 -right-0.5 flex h-6 w-6 items-center justify-center rounded-full bg-white shadow ring-2 ring-white">
                    <i data-lucide="circle-alert" class="h-3.5 w-3.5 text-[#600020]" stroke-width="2.5"></i>
                </span>
            </div>
            <div class="min-w-0 max-w-2xl flex-1 pt-0.5">
                <h2 class="text-balance text-2xl font-bold tracking-tight text-[#600020] sm:text-[1.65rem]">Indicadores Proativos de Segurança</h2>
                <p class="mt-2 max-w-prose text-sm leading-relaxed text-zinc-500">Ações preventivas e controle antecipado de riscos no período.</p>
        ...
        <div class="space-y-5 xl:col-span-4">
            <div class="rounded-[14px] border border-[#E0E0E0] bg-white p-5 shadow-sm sm:p-6">
                <div class="flex items-center gap-3">
                    ...
                    <h3 class="text-sm font-bold tracking-tight text-zinc-900">Ações proativas do período</h3>
                </div>
                <div class="mt-6 space-y-4">
                    @foreach ($barChartAcoes as $row)
                        <div class="flex items-center gap-3">
                            <div class="flex w-36 shrink-0 items-start gap-2 sm:w-40">
                                <i data-lucide="{{ $row['icon'] }}" class="mt-0.5 h-4 w-4 shrink-0 text-[#600020]" stroke-width="1.5"></i>
                ...
                <div class="grid grid-cols-2 gap-4 sm:grid-cols-4 sm:gap-2">
                    @foreach ($faixaRapida as $item)
                        <div class="flex flex-col items-center gap-2 text-center sm:flex-row sm:items-center sm:gap-3 sm:text-left">
                            ...
                            <div>
                                <p class="text-[11px] font-semibold leading-tight text-[#600020]">{{ $item['label'] }}</p>
                                <p class="text-lg font-bold tabular-nums text-[#600020]">{{ $item['value'] }}</p>
                            </div>
                        </div>
                    @endforeach
                </div>
        ...
        <div class="space-y-6 xl:col-span-8">
            ...
            <div class="grid gap-4 lg:grid-cols-2 lg:gap-5">
                <div class="rounded-[14px] border border-[#E0E0E0] bg-white p-5 shadow-sm sm:p-6">
                    ...
                    <p class="mt-4 text-sm font-normal leading-relaxed text-zinc-600">{{ $leituraProativos }}</p>
                </div>
                <div class="rounded-[14px] border border-[#E0E0E0] bg-white p-5 shadow-sm sm:p-6">
                    <div class="flex items-center gap-3">
                        <div class="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-[#f5e8ec]">
                    ...
                    <ul class="mt-4 list-inside list-disc space-y-2 text-sm font-normal leading-relaxed text-zinc-600">
                        @foreach ($pontosProativos as $ponto)
                            <li>{{ $ponto }}</li>
                        @endforeach
                    </ul>
                </div>
...
<div class="relative flex flex-wrap items-center gap-4 overflow-hidden rounded-b-2xl border-t border-white/10 bg-[#600020] px-6 py-4 text-white sm:px-8">
    ...
    <p class="relative min-w-0 flex-1 text-center text-xs font-bold uppercase tracking-[0.18em] sm:text-sm">Ações proativas de segurança evidenciadas</p>
