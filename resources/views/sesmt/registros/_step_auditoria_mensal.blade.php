@php
    $fill = $fill ?? [];
    $slug = 'auditoria_mensal';
    $passou = old("etapas.$slug.passou_auditoria", data_get($fill, "$slug.passou_auditoria", ''));
@endphp
<div class="space-y-5">
    <div class="rounded-xl border border-zinc-200 bg-white p-4 shadow-sm">
        <div class="grid gap-4 lg:grid-cols-[1fr_220px]">
            <div>
                <p class="text-xs font-bold uppercase tracking-wide text-brand-gray">O contrato passou por auditoria mensal?</p>
                <div class="mt-2 flex items-center gap-3">
                    <label class="inline-flex cursor-pointer items-center gap-2 rounded-lg border border-zinc-200 bg-white px-3 py-2 text-sm font-semibold text-brand-black">
                        <input type="radio" name="etapas[{{ $slug }}][passou_auditoria]" value="nao" @checked($passou === 'nao')>
                        Não
                    </label>
                    <label class="inline-flex cursor-pointer items-center gap-2 rounded-lg border border-zinc-200 bg-white px-3 py-2 text-sm font-semibold text-brand-black">
                        <input type="radio" name="etapas[{{ $slug }}][passou_auditoria]" value="sim" @checked($passou === 'sim')>
                        Sim
                    </label>
                </div>
            </div>
            <label>
                ...
                <input type="date" name="etapas[{{ $slug }}][data_auditoria]" value="{{ old("etapas.$slug.data_auditoria", data_get($fill, "$slug.data_auditoria")) }}" class="mt-2 h-11 w-full rounded-lg border border-zinc-200 bg-white px-3 text-sm outline-none transition focus:border-brand-burgundy focus:ring-2 focus:ring-brand-burgundy/10">
            </label>
        </div>

        <div class="mt-4 grid gap-4 md:grid-cols-2">
            <label>
                <span class="text-xs font-bold uppercase tracking-wide text-brand-gray">Auditor</span>
                <input type="text" name="etapas[{{ $slug }}][auditor]" value="{{ old("etapas.$slug.auditor", data_get($fill, "$slug.auditor")) }}" class="mt-2 h-11 w-full rounded-lg border border-zinc-200 bg-white px-3 text-sm outline-none transition focus:border-brand-burgundy focus:ring-2 focus:ring-brand-burgundy/10">
            </label>
            <label>
    ...
    @error("etapas.$slug.passou_auditoria")
        <p class="px-4 pb-2 text-sm font-medium text-red-600 sm:px-6">{{ $message }}</p>
    @enderror
    @error("etapas.$slug.evidencia_1")
        <p class="px-4 pb-2 text-sm font-medium text-red-600 sm:px-6">{{ $message }}</p>
    @enderror
    @error("etapas.$slug.evidencia_2")
        <p class="px-4 pb-2 text-sm font-medium text-red-600 sm:px-6">{{ $message }}</p>
    @enderror
</div>
