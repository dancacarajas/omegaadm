...
@section('title', 'Novo prazo SLA — SSMA')
@section('eyebrow', 'SSMA')
@section('page-title', 'Novo prazo (SLA)')
...
@section('content')
    <form method="POST" action="{{ route('sesmt.registros.prazos.store') }}" class="max-w-3xl space-y-6 rounded-xl border border-zinc-200 bg-white p-6 shadow-sm">
        @csrf
        ...
        <div class="flex justify-end gap-3 border-t border-zinc-100 pt-6">
            ...
            <button type="submit" class="inline-flex h-11 items-center rounded-xl bg-brand-burgundy px-5 text-sm font-semibold text-white shadow-sm shadow-brand-burgundy/20">Salvar prazo</button>
        </div>
    </form>
@endsection
