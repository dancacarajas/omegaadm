...
@section('actions')
    ...
    @if ($todosOk)
        <a href="{{ route('rh.beneficios.extrato.gerar') }}" class="inline-flex h-10 items-center gap-2 rounded-lg bg-brand-burgundy px-4 py-2 text-sm font-semibold text-white shadow-sm">
            <i data-lucide="file-text" class="h-4 w-4"></i>
            Gerar extrato
        </a>
    @endif
    <a href="{{ route('rh.beneficios.extrato.config') }}" class="inline-flex h-10 items-center gap-2 rounded-lg border border-zinc-200 bg-white px-4 py-2 text-sm font-semibold text-brand-black shadow-sm">
...
@section('content')
    ...
    <div class="mt-6 flex flex-wrap justify-end gap-3">
        @if ($todosOk)
            <a href="{{ route('rh.beneficios.extrato.gerar') }}" class="inline-flex h-11 items-center gap-2 rounded-xl bg-brand-burgundy px-5 text-sm font-semibold text-white">
                Ir para geração do extrato <i data-lucide="arrow-right" class="h-4 w-4"></i>
            </a>
        @else
    ...
    <script>
        ...
        document.querySelectorAll('[data-abrir-modal-va]').forEach((btn) => {
            btn.addEventListener('click', () => abrirModal('modal-va-' + btn.getAttribute('data-abrir-modal-va')));
        });
        document.querySelectorAll('[data-abrir-modal-cafe]').forEach((btn) => {
            btn.addEventListener('click', () => abrirModal('modal-cafe-' + btn.getAttribute('data-abrir-modal-cafe')));
        });
