...
@section('content')
        ...
        <div class="relative flex flex-col gap-6 p-6 sm:flex-row sm:items-end sm:justify-between sm:p-8">
            <div>
                ...
                <p class="mt-2 max-w-xl text-sm leading-relaxed text-brand-burgundy-soft/90">
                    Gerencie processos pendentes, finalize afastamentos e consulte o histórico do efetivo.
                </p>
            </div>
            <div class="flex shrink-0 flex-wrap gap-3 sm:justify-end">
                ...
                <div class="min-w-[5.5rem] rounded-2xl border border-white/20 bg-white/10 px-5 py-3 text-center backdrop-blur-sm">
                    <p class="text-[10px] font-bold uppercase tracking-wider text-brand-burgundy-soft/80">Pendentes</p>
                    <p class="mt-0.5 text-2xl font-bold tabular-nums text-white">{{ $resumo['pendentes'] }}</p>
                </div>
            </div>
        </div>
    </section>
    ...
    <section class="mb-6 grid grid-cols-2 gap-4 lg:grid-cols-4">
        ...
        <article class="rounded-2xl border border-zinc-200/80 bg-white p-5 shadow-sm ring-1 ring-zinc-100 transition hover:shadow-md">
            ...
            <p class="mt-4 text-xs font-bold uppercase tracking-wider text-zinc-500">Pendentes de finalização</p>
            <p class="mt-1 text-3xl font-bold tracking-tight text-zinc-900">{{ $resumo['pendentes'] }}</p>
        </article>
        <article class="rounded-2xl border border-zinc-200/80 bg-white p-5 shadow-sm ring-1 ring-zinc-100 transition hover:shadow-md">
            <span class="flex h-11 w-11 items-center justify-center rounded-2xl bg-brand-burgundy-soft text-brand-burgundy">
                <i data-lucide="heart-pulse" class="h-5 w-5"></i>
            </span>
            <p class="mt-4 text-xs font-bold uppercase tracking-wider text-zinc-500">INSS pendentes</p>
            <p class="mt-1 text-3xl font-bold tracking-tight text-zinc-900">{{ $resumo['afastamento_inss_pendentes'] }}</p>
        </article>
    </section>
        ...
        <div class="border-b border-zinc-100 bg-gradient-to-r from-brand-burgundy/[0.04] via-zinc-50/90 to-white px-6 py-5">
            ...
            <form method="GET" action="{{ route('rh.efetivo.movimentacoes.index') }}" class="mt-5 rounded-2xl border border-zinc-200/80 bg-white p-4 shadow-sm">
                <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-12 lg:items-end">
                    ...
                    <label class="space-y-2 lg:col-span-3">
                        <span class="flex items-center gap-1.5 text-[11px] font-bold uppercase tracking-wider text-zinc-400">
                            <i data-lucide="clipboard-list" class="h-3.5 w-3.5 text-brand-burgundy/70"></i>
                            Situação
                        </span>
                        <select name="situacao" class="h-12 w-full rounded-2xl border border-zinc-200/90 bg-zinc-50/50 px-4 text-sm font-medium text-zinc-900 outline-none transition focus:border-brand-burgundy focus:bg-white focus:ring-4 focus:ring-brand-burgundy/10">
                            <option value="">Todas</option>
                            @foreach ($situacoes as $key => $label)
                                <option value="{{ $key }}" @selected($situacaoFiltro === $key)>{{ $label }}</option>
                            @endforeach
                        </select>
                    </label>
                ...
                @if ($filtrosAtivos)
                    <div class="mt-4 flex flex-wrap items-center gap-2 border-t border-zinc-100 pt-4">
                        ...
                        @endif
                        @if (filled($situacaoFiltro))
                            <span class="inline-flex rounded-full bg-amber-50 px-2.5 py-1 text-xs font-semibold text-amber-800">{{ $situacoes[$situacaoFiltro] ?? $situacaoFiltro }}</span>
                        @endif
                    </div>
                @endif
            </form>
                        ...
                        <tr class="transition hover:bg-brand-burgundy/[0.02]">
                            ...
                            <td class="px-5 py-4 text-right">
                                <div class="flex flex-wrap justify-end gap-2">
                                    @if ($mov->isPendente())
                                        <form method="POST" action="{{ route('rh.efetivo.movimentacoes.finalizar', $mov) }}" class="inline" onsubmit="return confirm('Finalizar este processo e aplicar no cadastro do colaborador?')">
                                            @csrf
                                            <button type="submit" class="inline-flex h-9 items-center gap-1 rounded-xl bg-emerald-600 px-3 text-xs font-bold text-white shadow-sm hover:bg-emerald-700">
