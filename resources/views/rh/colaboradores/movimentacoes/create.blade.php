...
@php
    $editando = isset($movimentacao);
@endphp

@section('title', $editando ? 'Alterar movimentação - Omega286' : 'Registrar movimentação - Omega286')
@section('eyebrow', 'Recursos Humanos / Efetivo')
@section('page-title', $editando ? 'Alterar movimentação' : 'Registrar movimentação')
...
@section('content')
        ...
        <section class="rounded-xl border border-zinc-200 bg-white p-5 shadow-sm sm:p-6">
            ...
            <p class="mt-1 text-sm text-brand-gray">
                @if ($tipo === 'afastamento_inss')
                    Registre o afastamento previdenciário. Para <strong>acidente de trabalho</strong>, use a espécie correta — o extrato de vale alimentação usa essa movimentação para isentar desconto por falta nos primeiros 3 meses.
                @else
                    O cadastro do colaborador será atualizado automaticamente após salvar.
                @endif
            </p>
            ...
            <div class="mt-6 grid gap-5 sm:grid-cols-2">
                <div>
                    <label for="data_inicio" class="block text-xs font-bold uppercase tracking-wide text-brand-gray">
                        ...
                        @elseif ($tipo === 'afastamento_inss')
                            Início do afastamento
                        @else
                            Data de início
                        @endif
                    </label>
                    <input type="date" name="data_inicio" id="data_inicio" value="{{ old('data_inicio', $editando ? $movimentacao->data_inicio?->format('Y-m-d') : today()->format('Y-m-d')) }}" required class="mt-2 h-11 w-full rounded-lg border border-zinc-200 px-3 text-sm focus:border-brand-burgundy focus:ring-2 focus:ring-brand-burgundy/20">
                ...
                @if (in_array($tipo, ['ferias', 'afastamento_inss'], true))
                    <div>
                        <label for="data_fim" class="block text-xs font-bold uppercase tracking-wide text-brand-gray">
                            @if ($tipo === 'ferias')
                                Data fim <span class="text-red-600">*</span>
                            @else
                                Previsão de alta / data fim
                            @endif
                        </label>
                        <input type="date" name="data_fim" id="data_fim" value="{{ old('data_fim', $editando ? $movimentacao->data_fim?->format('Y-m-d') : '') }}" @if ($tipo === 'ferias') required @endif class="mt-2 h-11 w-full rounded-lg border border-zinc-200 px-3 text-sm focus:border-brand-burgundy focus:ring-2 focus:ring-brand-burgundy/20">
                        @if ($tipo === 'afastamento_inss')
                            <p class="mt-1 text-xs text-brand-gray">Deixe em branco se o afastamento ainda estiver em aberto.</p>
                        @endif
            ...
            @if ($tipo === 'desligamento')
                <div class="mt-5 grid gap-5 sm:grid-cols-2">
                    <div>
                        ...
                        <select name="tipo_rescisao" id="tipo_rescisao" required class="mt-2 h-11 w-full rounded-lg border border-zinc-200 px-3 text-sm focus:border-brand-burgundy focus:ring-2 focus:ring-brand-burgundy/20">
                            ...
                            @foreach ($tiposRescisao as $key => $label)
                                <option value="{{ $key }}" @selected(old('tipo_rescisao', $editando ? $movimentacao->tipo_rescisao : null) === $key)>{{ $label }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="sm:col-span-2">
                        ...
                        <input type="text" name="motivo_texto" id="motivo_texto" value="{{ old('motivo_texto', $editando ? $movimentacao->motivo_texto : '') }}" maxlength="500" class="mt-2 h-11 w-full rounded-lg border border-zinc-200 px-3 text-sm focus:border-brand-burgundy focus:ring-2 focus:ring-brand-burgundy/20">
                    </div>
                </div>
            @endif
            ...
            @if ($tipo === 'transferencia_contrato')
                <div class="mt-5 grid gap-5 sm:grid-cols-2">
                    ...
                    <div>
                        ...
                        <input type="text" name="centro_custo_novo" id="centro_custo_novo" value="{{ old('centro_custo_novo', $editando ? $movimentacao->centro_custo_novo : '') }}" list="centros-custo-list" required maxlength="80" class="mt-2 h-11 w-full rounded-lg border border-zinc-200 px-3 text-sm focus:border-brand-burgundy focus:ring-2 focus:ring-brand-burgundy/20">
                    ...
                    <div>
                        ...
                        <input type="text" name="tipo_contrato_novo" id="tipo_contrato_novo" value="{{ old('tipo_contrato_novo', $editando ? $movimentacao->tipo_contrato_novo : $colaborador->tipo_contrato) }}" maxlength="80" class="mt-2 h-11 w-full rounded-lg border border-zinc-200 px-3 text-sm focus:border-brand-burgundy focus:ring-2 focus:ring-brand-burgundy/20">
                    </div>
                    <div>
                        ...
                        <input type="text" name="local_trabalho_novo" id="local_trabalho_novo" value="{{ old('local_trabalho_novo', $editando ? $movimentacao->local_trabalho_novo : $colaborador->local_trabalho) }}" maxlength="255" class="mt-2 h-11 w-full rounded-lg border border-zinc-200 px-3 text-sm focus:border-brand-burgundy focus:ring-2 focus:ring-brand-burgundy/20">
                    </div>
                    <div class="sm:col-span-2">
                        <label for="departamento_novo" class="block text-xs font-bold uppercase tracking-wide text-brand-gray">Departamento</label>
                        <input type="text" name="departamento_novo" id="departamento_novo" value="{{ old('departamento_novo', $editando ? $movimentacao->departamento_novo : $colaborador->departamento) }}" maxlength="255" class="mt-2 h-11 w-full rounded-lg border border-zinc-200 px-3 text-sm focus:border-brand-burgundy focus:ring-2 focus:ring-brand-burgundy/20">
                    </div>
                </div>
            @endif
            ...
            @if ($tipo === 'promocao')
                <div class="mt-5 grid gap-5 sm:grid-cols-2">
                    ...
                    <div>
                        ...
                        <input type="text" name="cargo_novo" id="cargo_novo" value="{{ old('cargo_novo', $editando ? $movimentacao->cargo_novo : '') }}" required maxlength="255" class="mt-2 h-11 w-full rounded-lg border border-zinc-200 px-3 text-sm focus:border-brand-burgundy focus:ring-2 focus:ring-brand-burgundy/20">
                    ...
                    <div>
                        ...
                        <input type="number" name="salario_novo" id="salario_novo" value="{{ old('salario_novo', $editando ? $movimentacao->salario_novo : '') }}" min="0" step="0.01" class="mt-2 h-11 w-full rounded-lg border border-zinc-200 px-3 text-sm focus:border-brand-burgundy focus:ring-2 focus:ring-brand-burgundy/20">
                    </div>
                    <div class="sm:col-span-2">
                        <label for="departamento_novo" class="block text-xs font-bold uppercase tracking-wide text-brand-gray">Departamento</label>
                        <input type="text" name="departamento_novo" id="departamento_novo" value="{{ old('departamento_novo', $editando ? $movimentacao->departamento_novo : $colaborador->departamento) }}" maxlength="255" class="mt-2 h-11 w-full rounded-lg border border-zinc-200 px-3 text-sm focus:border-brand-burgundy focus:ring-2 focus:ring-brand-burgundy/20">
            ...
            @if ($tipo === 'ferias')
                <div class="mt-5 grid gap-5 sm:grid-cols-2">
                    <div>
                        ...
                        <input type="number" name="dias_ferias" id="dias_ferias" value="{{ old('dias_ferias', $editando ? $movimentacao->dias_ferias : '') }}" min="1" max="60" class="mt-2 h-11 w-full rounded-lg border border-zinc-200 px-3 text-sm focus:border-brand-burgundy focus:ring-2 focus:ring-brand-burgundy/20">
                    ...
                    <div class="flex items-end pb-1">
                        <label class="inline-flex cursor-pointer items-center gap-2 text-sm font-semibold text-brand-black">
                            ...
                            <input type="checkbox" name="abono_pecuniario" value="1" class="h-4 w-4 rounded border-zinc-300 text-brand-burgundy" @checked(old('abono_pecuniario', $editando ? $movimentacao->abono_pecuniario : false))>
            ...
            @if ($tipo === 'afastamento_inss')
                <div class="mt-5 grid gap-5 sm:grid-cols-2">
                    <div class="sm:col-span-2">
                        <label for="especie_beneficio_inss" class="block text-xs font-bold uppercase tracking-wide text-brand-gray">Espécie do benefício INSS</label>
                        <select name="especie_beneficio_inss" id="especie_beneficio_inss" required class="mt-2 h-11 w-full rounded-lg border border-zinc-200 px-3 text-sm focus:border-brand-burgundy focus:ring-2 focus:ring-brand-burgundy/20">
                            ...
                            @foreach ($especiesInss as $key => $label)
                                <option value="{{ $key }}" @selected(old('especie_beneficio_inss', $editando ? $movimentacao->especie_beneficio_inss : null) === $key)>{{ $label }}</option>
                            @endforeach
                        </select>
                        @error('especie_beneficio_inss')<p class="mt-1 text-xs font-semibold text-red-600">{{ $message }}</p>@enderror
                    </div>
                    <div id="aviso-acidente-trabalho" class="hidden sm:col-span-2 rounded-xl border border-blue-200 bg-blue-50 px-4 py-3 text-sm text-blue-900">
                        <p class="font-semibold">Acidente de trabalho e vale alimentação</p>
                        <p class="mt-1 text-xs leading-relaxed text-blue-800">
                            Nos meses em que o colaborador estiver afastado por esta espécie, o extrato de benefícios pode aplicar o vale alimentação <strong>sem desconto por falta</strong>, até o limite configurado nas regras do extrato (padrão: 3 meses contados a partir da data de início do afastamento).
                        </p>
                    </div>
                    <div>
                        <label for="cid" class="block text-xs font-bold uppercase tracking-wide text-brand-gray">CID (opcional)</label>
                        <input type="text" name="cid" id="cid" value="{{ old('cid', $editando ? $movimentacao->cid : '') }}" maxlength="20" class="mt-2 h-11 w-full rounded-lg border border-zinc-200 px-3 text-sm focus:border-brand-burgundy focus:ring-2 focus:ring-brand-burgundy/20">
                    </div>
                </div>
            @endif
            ...
            <div class="mt-5">
                ...
                <textarea name="observacoes" id="observacoes" rows="3" class="mt-2 w-full rounded-lg border border-zinc-200 px-3 py-2 text-sm focus:border-brand-burgundy focus:ring-2 focus:ring-brand-burgundy/20">{{ old('observacoes', $editando ? $movimentacao->observacoes : '') }}</textarea>
            </div>
        </section>
    ...
    @if ($tipo === 'afastamento_inss')
        @push('scripts')
            <script>
                (function () {
                    const select = document.getElementById('especie_beneficio_inss');
                    const aviso = document.getElementById('aviso-acidente-trabalho');
                    if (!select || !aviso) return;
                    const atualizar = () => {
                        aviso.classList.toggle('hidden', select.value !== 'acidente_trabalho');
                    };
